...
class BME280Spi: public BME280{
  ...
  bool ReadAddrHwSpi(uint8_t addr, uint8_t array[], uint8_t len);
  bool ReadAddrSwSpi(uint8_t addr, uint8_t array[], uint8_t len);

  /* ==== Write values to BME280 registers. ==== */
  virtual void WriteRegister(uint8_t addr, uint8_t data);
  virtual void WriteRegisterHwSpi(uint8_t addr, uint8_t data);
  virtual void WriteRegisterSwSpi(uint8_t addr, uint8_t data);
...
public:
  ...
  BME280Spi(uint8_t spiCsPin, uint8_t spiMosiPin, uint8_t spiMisoPin, uint8_t spiSckPin, uint8_t tosr = 0x1,
